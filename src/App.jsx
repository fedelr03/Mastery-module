...
function AuthScreen({ confirmedEmail }) {
  ...
  // Show confirmed message when coming from email link, update when lang changes
  useEffect(() => {
    ...
    try {
      if (isLogin) {
        ...
        if (data?.user?.identities?.length === 0) {
          ...
          // Store username locally — applied after email confirmation + sign-in
...
/* ═══════════ PROFILE SCREEN ═══════════ */
function ProfileScreen({ session, profile, onSave }) {
  const [username, setUsername] = useState(profile?.username || '');
  const [interests, setInterests] = useState(profile?.interests || '');
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  // Sync if parent profile prop loads after mount
  useEffect(() => {
    setUsername(profile?.username || '');
    setInterests(profile?.interests || '');
  }, [profile?.username, profile?.interests]);

  const handleSave = async () => {
    setSaving(true);
    const { error } = await supabase
      .from('profiles')
      .update({ username: username.trim(), interests: interests.trim() })
      .eq('id', session.user.id);
    if (!error) {
      onSave({ username: username.trim(), interests: interests.trim() });
      setSaved(true);
  ...
  return (
    <div style={{ maxWidth: 520, margin: '0 auto', padding: '32px 20px 60px', animation: 'fadeUp 0.3s ease' }}>
      <div style={{ marginBottom: 28 }}>
        <h2 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: 24, fontWeight: 800, color: TH.text, marginBottom: 4 }}>My Profile</h2>
        <p style={{ color: TH.textMuted, fontSize: 12 }}>Customize how you appear in the app</p>
      </div>
      ...
      <div style={{ background: TH.surface, borderRadius: 16, border: '1px solid ' + TH.border, padding: 24, boxShadow: '0 4px 24px rgba(0,0,0,0.04)' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <label style={{ fontSize: 11, fontWeight: 600, color: TH.textSecondary, display: 'block', marginBottom: 5 }}>
            USERNAME{' '}
            <span style={{ color: TH.textFaint, fontWeight: 400 }}>— replaces your email in the top bar</span>
          </label>
          <input type="text" value={username} onChange={e => setUsername(e.target.value)}
            placeholder="e.g. Federico, Profe, fede.lr"
            style={inputStyle}
            onFocus={e => e.target.style.borderColor = TH.accent}
            onBlur={e => e.target.style.borderColor = TH.border} />
        </div>
        ...
        <div style={{ marginBottom: 28 }}>
          <label style={{ fontSize: 11, fontWeight: 600, color: TH.textSecondary, display: 'block', marginBottom: 5 }}>
            INTERESTS{' '}
            <span style={{ color: TH.textFaint, fontWeight: 400 }}>— coming soon: personalized examples & analogies</span>
          </label>
          <textarea value={interests} onChange={e => setInterests(e.target.value)}
            placeholder="e.g. Football, The Simpsons, investing, Formula 1…"
            rows={3}
...
/* ═══════════ TOP BAR ═══════════ */
function TopBar({ session, profile, isAdmin, view, setView, onLogout }) {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // After a user clicks their confirmation link, Supabase redirects back
    // with hash params. We detect this to show a friendly "confirmed" message
    // and clean up the URL so there's no ugly hash in the address bar.
    ...
    if (hash) {
      ...
      // Always clear the hash — whether it's a success or an error token.
      // Supabase JS client handles the token exchange via getSession() below.
  ...
  // If the user signed up with a username but hadn't confirmed yet,
  // this applies it the first time they successfully log in.
  ...
  const loadProfile = async (userId) => {
    const { data } = await supabase.from('profiles').select('username, interests').eq('id', userId).single();
    setProfile(data || {});
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: TH.bg }}>
      <TopBar session={session} profile={profile} isAdmin={isAdmin} view={view} setView={setView} onLogout={handleLogout} />
      <div style={{ paddingTop: 48 }}>
        {view === 'admin' && isAdmin
          ? <AdminDashboard />
          : view === 'profile'
          ? <ProfileScreen session={session} profile={profile} onSave={handleProfileSave} />
