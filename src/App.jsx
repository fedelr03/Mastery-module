      ...
      <div style={{ background: TH.surface, borderRadius: 16, border: '1px solid ' + TH.border, padding: 24, boxShadow: '0 4px 24px rgba(0,0,0,0.04)' }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', gap: 0, borderRadius: 10, overflow: 'hidden', border: '1px solid ' + TH.border }}>
            {[
              { value: 'beginner', emoji: '🌱', label: 'Beginner', desc: 'Simple language, more examples' },
              { value: 'intermediate', emoji: '📖', label: 'Intermediate', desc: 'Balanced depth & clarity' },
              { value: 'advanced', emoji: '🔬', label: 'Advanced', desc: 'Technical, skips the basics' },
            ].map((opt, i) => (
    ...
    <div style={{ minHeight: '100vh', background: TH.bg }}>
      ...
      <div style={{ paddingTop: 48 }}>
        {view === 'admin' && isAdmin
          ...
          : <div style={{ maxWidth: 960, margin: '0 auto' }}><MasteryModule session={session} level={profile?.level || 'intermediate'} /></div>
        }
      </div>
    </div>
  );
